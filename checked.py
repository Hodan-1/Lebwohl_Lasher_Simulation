...
def all_energy(arr, nmax):
 ...
 Args:
    ...
    energy_local = np.sum(0.5 * (1.0 - 3.0 * np.cos(arr - neighbors) ** 2))
...
def MC_step(arr, Ts, nmax):
    """
    Perform one Monte Carlo step with checkerboard updates using NumPy vectorization.
    """
    scale = 0.1 + Ts
    accept = 0
...
def plotdat(arr, pflag, nmax):
    ...
    ax.quiver(x, y, u, v, scale=20, pivot='middle', color='black')
    ax.set_aspect('equal')
    plt.show()
...
def main(nsteps, nmax, temp, pflag):
    ...
    # Final output (only by root process)
    if rank == 0:
